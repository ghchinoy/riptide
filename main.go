...
func main() {
	...
	serve := flag.Bool("serve", false, "Start the Session Viewer web UI in the background.")
	flag.Parse()
	...
	if *serve {
		go func() {
			// We don't import viewer at the top to avoid cyclic dependencies if any, but let's add it to the imports
			// Actually, I'll let goimports handle the import block if I use a bash command, or I can add it explicitly.
			if err := viewer.Start(":8083"); err != nil {
				log.Printf("Session viewer server error: %v", err)
			}
		}()
	}
